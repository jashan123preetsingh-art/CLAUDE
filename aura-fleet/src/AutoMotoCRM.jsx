...
const printDoc = (title, text) => {
  const w = window.open("", "_blank", "width=760,height=920");
  if (!w) return;
  w.document.write(`<!doctype html><html><head><title>${title}</title><style>
    body{font-family:ui-monospace,Menlo,Consolas,monospace;font-size:12px;line-height:1.7;color:#18181b;padding:36px;max-width:640px;margin:0 auto}
    .hd{font-family:system-ui,sans-serif;border-bottom:2px solid #18181b;padding-bottom:12px;margin-bottom:20px}
    .hd b{font-size:16px}.hd div{color:#71717a;font-size:11px;margin-top:3px}
    pre{white-space:pre-wrap;word-break:break-word}
    .ft{margin-top:28px;border-top:1px solid #e4e4e7;padding-top:8px;color:#a1a1aa;font-size:10px;font-family:system-ui,sans-serif}
    @media print{body{padding:12px}}
  </style></head><body>
    <div class="hd"><b>${COMPANY_LEGAL}</b><div>${COMPANY_ADDRESS} · GSTIN: ${COMPANY_GSTIN} · ${COMPANY_CONTACT}</div></div>
    <pre>${text.replace(/</g, "&lt;")}</pre>
    <div class="ft">${title} · generated by the ${COMPANY} fleet CRM</div>
  </body></html>`);
  w.document.close();
  w.focus();
  setTimeout(() => { try { w.print(); } catch (e) { /* user closed */ } }, 350);
};
...
const SCHEMA_V = "2026-07-12.2"; // bump when saved-data shape changes
try {
  if (localStorage.getItem("amms.schema") !== SCHEMA_V) {
    Object.keys(localStorage).filter((k) => k.startsWith("amms.") && k !== "amms.schema").forEach((k) => localStorage.removeItem(k));
    localStorage.setItem("amms.schema", SCHEMA_V);
  }
    ...
    <div className="space-y-3">
      ...
      {add && (
        <Modal title="Add car" onClose={() => setAdd(false)}>
          ...
          {!f.reg.trim() && <p className="mt-2 text-xs font-medium text-rose-600">Enter the registration number — the Save button unlocks after that.</p>}
          <div className="mt-3 flex justify-end gap-2"><Btn kind="ghost" onClick={() => setAdd(false)}>Cancel</Btn><Btn kind="accent" disabled={!f.reg.trim()} onClick={saveCar}>Save car</Btn></div>
        </Modal>
      )}
    </div>
  );
    ...
    <div className="space-y-3">
        ...
        return (
          <Card key={d.id} className={d.active ? "" : "opacity-60"}>
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-4">
              ...
              <div><span className="text-zinc-500">On apps: </span><span className="font-medium">{(d.apps || []).join(" · ") || "—"}</span></div>
              <div><span className="text-zinc-500">Daily duty: </span><span style={MONO}>{d.appHrs || 0} hr apps{d.corpHrs ? ` + ${d.corpHrs} hr corporate` : ""}</span></div>
            </div>
            {d.history.length > 0 && (
            ...
            {earnOpen === d.id && (() => {
              ...
              return (
                <div className="mt-3 rounded-xl border border-zinc-900/[0.06] bg-white p-3.5">
                  ...
                  <p className="mt-2 text-[11.5px] text-zinc-400">Built automatically from recorded settlements — record every week's gross + fuel and this becomes the driver's full performance record. Benchmark (verified, Ludhiana/Punjab tier-1): a salaried private driver earns ₹11–15k/mo; a full-time taxi driver averages ~₹25–29k. Keep full-day drivers in the ₹16–20k+ band — below ~₹14k after 2 full months he will leave for a salaried seat; feed corporate runs or coach his hours.</p>
                </div>
              );
            })()}
      ...
      {add && (
        <Modal title="Add driver" onClose={() => setAdd(false)}>
          ...
          {!f.name.trim() && <p className="mt-2 text-xs font-medium text-rose-600">Enter the driver's full name — the Save button unlocks after that.</p>}
          <div className="mt-3 flex justify-end gap-2"><Btn kind="ghost" onClick={() => setAdd(false)}>Cancel</Btn><Btn kind="accent" disabled={!f.name.trim()} onClick={save}>Save driver</Btn></div>
        </Modal>
      )}
      {duty && (
        <Modal title={`Deal & duty · ${duty.name}`} onClose={() => setDuty(null)}>
          ...
          <div className="mb-3">
            ...
            <div className="flex gap-2">
              {APPS_LIST.map((a) => (
                <button key={a} onClick={() => setDApps(dApps.includes(a) ? dApps.filter((x) => x !== a) : [...dApps, a])}
                  className={`rounded-full px-3.5 py-1.5 text-[12px] font-semibold ring-1 ring-inset transition-all duration-200 focus:outline-none focus-visible:ring-4 focus-visible:ring-amber-400/30 ${dApps.includes(a) ? "bg-zinc-900 text-white ring-zinc-900" : "bg-white text-zinc-500 ring-zinc-200 hover:text-zinc-900"}`}>
                  {a}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Avg app hours / day"><NumIn value={dAppHrs} step={0.5} onChange={setDAppHrs} /></Field>
            <Field label="Corporate hours / day"><NumIn value={dCorpHrs} step={0.5} onChange={setDCorpHrs} /></Field>
          </div>
          <div className="mt-3">
            <Field label="Corporate assignment (best effort · not guaranteed)">
              <select className={inputCls} value={dClient} onChange={(e) => setDClient(e.target.value)}>
                <option value="">— none —</option>
                {clients.map((k) => <option key={k.id} value={k.id}>{k.name}</option>)}
              </select>
            </Field>
          </div>
          <div className="mt-3 rounded-lg bg-zinc-50 p-2 text-xs text-zinc-500">Total duty: <span style={MONO} className="font-semibold text-zinc-900">{(dAppHrs || 0) + (dCorpHrs || 0)} hr/day</span> · 10–12 hr is the healthy full-time band; above 13 hr watch fatigue.</div>
          <div className="mt-3 flex justify-end gap-2"><Btn kind="ghost" onClick={() => setDuty(null)}>Cancel</Btn><Btn kind="accent" onClick={saveDuty}>Save</Btn></div>
        </Modal>
      )}
      {settle && (() => {
      ...
      {doc && (
        <Modal title={doc.title} onClose={() => setDoc(null)} wide>
          ...
          <div className="mt-3 flex justify-end gap-2"><Btn kind="ghost" onClick={() => setDoc(null)}>Close</Btn><Btn kind="ghost" onClick={() => copyDoc(doc.text)}>Copy text</Btn><Btn kind="accent" onClick={() => printDoc(doc.title, doc.text)}>🖨 Print slip</Btn></div>
        </Modal>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      {doc && (
        <Modal title={doc.title} onClose={() => setDoc(null)} wide>
          ...
          <div className="mt-4 flex justify-end gap-2"><Btn kind="ghost" onClick={() => setDoc(null)}>Close</Btn><Btn kind="ghost" onClick={() => copyDoc(doc.text)}>Copy text</Btn><Btn kind="accent" onClick={() => printDoc(doc.title, doc.text)}>🖨 Print slip</Btn></div>
        </Modal>
      )}
